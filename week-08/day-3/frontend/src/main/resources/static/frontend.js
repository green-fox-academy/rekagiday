...
$("#dountil").on("click", function () {
  ...
  $.post(window.baseUrl + "/dountil/sum", {}).done(function (data) {
    if("error" in data && data["error"] == "Please provide a model!") {
      handle("#dountil_response", "OK - without model");
    } else {
      handle("#dountil_response", "not ok - without model", "Dountil", data);
    }
  }).error(function (err) {
    handle("#dountil_response", "not ok - without model", "Dountil", err);
  });
